function [window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODR_030_prandHelper(Display, vstruct, ClassStructure)
...
for m  = 1:length(ClassStructures)
    for n = 1:length(ClassStructures(m).frame)
        ...
        for o = 1:numel(ClassStructures(m).frame(n).stim)
            Screen(WindowStructure(m).frame(n).end,'FillRect',Display.Lum(n)*WhiteIndex(0) * ClassStructure(m).frame(n).Lum,ClassStructures(m).frame(n).stim(o).wRect);
        end
