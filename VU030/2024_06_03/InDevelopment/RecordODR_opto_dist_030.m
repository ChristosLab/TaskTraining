...
Version = 'RecordODR_opto_dist_030_v0.1_2023_12_08' ; % after code changes, change version
...
datain(11) = 0;                % distractor luminance as percentage (1 - 100) of color depth (typically 0 - 255)
...
%   Laser Settings
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
visual_distract = false; % Whether to arrange multiple classes per cue location by queueing different visual distractors
laser_frame = 4; % Laser start in the distractor window
ao_mod_params = struct;
ao_mod_params.mod_type = 'sine'; % Modulation function
ao_mod_params.freq = 1/datain(2); % Modulation frequncy (Hz)
ao_mod_params.amp = 0; % Modulation voltage (V, 0 to max)
ao_mod_params.T = datain(2); % Laser on duration (second)
ao_mod_params.T_delay = 0.041;  % Laser delay duration (second); hard-coded minimum estimate = 0.04
...
[ClassStructure] = CreateClassStructure_ODR_opto_dist_030(stim_radius, cue_loc, visual_distract);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Visual settings
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
AllData.parameters.visual_distract = visual_distract;
...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    while (BreakState ~= 1) && (block_counter <= total_blocks)
        ...
        IndexTotl = randperm(length(ClassStructure)); % Where initial trials are shuffled in a block.
        CurrentClass = IndexTotl(1);
        while (repeat_counter <= total_trials) && (BreakState ~= 1) % Each trial
            currentStim  = ClassStructure(CurrentClass).laser
            %   Queue desired laser output
            if currentStim
