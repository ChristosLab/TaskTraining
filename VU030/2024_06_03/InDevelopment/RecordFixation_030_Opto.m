% RecordFixation_030_Opto.m Based on RecordODR_030_Opto.m. 
% Added photodiode event to all frames to sigal fixation on, via
% CreateWindowStructFixation_030 ZW 20230519
clear
close all
% warning off all
sca;
...
%%  Version info
version = 'RecordFixation_030_Opto_v0.1' ; % after code changes, change version
%   New WaveInitDaq
%   Added laser output                                  2023_05_17 -ZW
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datain(1:4) = [2 0 0 0];    %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
...
ao_mod_params.amp = 5; % Modulation voltage (V, 0 to max)
ao_mod_params.T = 0.6; % Laser on duration (second)
ao_mod_params.T_total = datain(laser_frame);  % Laser possible duration (second)
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructFixation_030(Display, vstruct, GeneralVars.ClassStructure);
...
% try
    while (BreakState ~= 1) && (blockcounter <= totalblocks)
        ...
        while (repeatcounter <= totaltrials) && (BreakState ~=1)
            ...
            queue_jittered_analog_output(AO, ao_mod_params);
