...
%%  Version info
version = 'TrainODR_030_v0.12_2024_03_18' ; % after code changes, change version
%   Added AllData.parameter.script variable to save the current script as
%   characters in the output data file                     2024_03_18 -RM
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datain(1:4) = [1 0.5 3.0 0.2];  %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
...
extra = 0.5;                    % extra window size for upper left corner
numBurst = 2;
...
[vstruct, Display] = WaveDisplayParamsTrinity_030(vstruct, datain, extra);
...
TargWindowSizeTrinity(1,:) = (datain(9)+extra)*Figdata.xcoord;
TargWindowSizeTrinity(2,:) = (datain(9)+extra)*Figdata.ycoord;
...
AllData.parameters.targetWindowUpperLeft = datain(9)+extra;
...
%              UpdateEyeDisplay(AllCoordinates.fRect, FixWindowSize, AllCoordinates.fRect,vstruct, hLine,'off')
            if CurrentClass == 4
                eye_display_queue         = {... 
                    ...
                    AllCoordinates.fRect, FixWindowSize; ...
                    AllCoordinates.fRect, FixWindowSize; ...
                    AllCoordinates.cRect(CurrentClass, :, 2), TargWindowSizeTrinity};
            else
                eye_display_queue         = {... 
                    AllCoordinates.fRect, FixWindowSize; ...
                    AllCoordinates.fRect, FixWindowSize; ...
            ...
            if CurrentClass == 4
                eye_target_queue      = {...
                    ...
                    GeneralVars.ClassStructure(CurrentClass).frame(1).stim(1).end, [Display.TargetWindowTrinity]};    % Target epoch
            else
                eye_target_queue      = {...
                    [0, 0], [Display.FixationWindow]; ... % Fixation epoch
                    [0, 0], [Display.FixationWindow]; ... % Cue epoch
