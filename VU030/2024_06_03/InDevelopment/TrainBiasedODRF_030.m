% TrainBiasedODRF_030
% based on TrainbiasedODR_030. JZ 20230130
clear
close all
% warning off all
sca;
...
Version = 'TrainBiasedODRF_030_v0.1_01_30_22' ; % after code changes, change version
%% Parameters
loc_mean = 134;                 % In degree, change before run
...
[ClassStructure, p, alpha, AllList] = CreateClassStructure_ODRF_vm_030(stim_radius, n_class, loc, kappa, w);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Visual settings
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ReactionTime = 0;
% Stimulus Windows
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODRF_030(Display, vstruct, ClassStructure);
...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    ...
    while (repeat_counter <= total_trials) && (BreakState ~= 1) % each trial
        ...
        frame_time_queue = [datain(1:4),0.2];         %   {'Fixation'; 'Cue'; 'Delay'; 'Target';'feedback'}
        ...
        eye_display_queue = {...
            ...
            AllCoordinates.cRect(CurrentClass, :, 2), FbWindowSize; ...
            AllCoordinates.fRect, FixWindowSize; ...
            AllCoordinates.cRect(CurrentClass, :, 2), TargWindowSize;
            AllCoordinates.cRect, FbWindowSize};
        ...
        eye_target_queue = {...
            ...
            ClassStructure(CurrentClass).frame(1).stim(1).end, [Display.TargetWindow];
            [0, 0], 0};    % Target epoch
        time_stamp_queue = [];
        Statecode = 0;
        FixState = 0;
        ...
        finish_Statecode = sum(aqusition_time_queue > 0) + numel(aqusition_time_queue);% exclude feedack frame
