function [window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODR_030_randHelper(Display, vstruct, ClassStructure)
% For use with TrainODRDist_randHelper.
% add random on/off for helpers(targets). -- J Zhu 20220831
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for m  = 1:length(ClassStructures)
    Display.RandLum = Display.Lum;
    Display.RandLum(3) = Display.RandLum(3)*(randi(2)-1); % random on/off for the helper -- JZ
    for n = 1:length(ClassStructures(m).frame)
        ...
        for o = 1:numel(ClassStructures(m).frame(n).stim)
            Screen(WindowStructure(m).frame(n).end,'FillRect',Display.RandLum(n)*WhiteIndex(0),ClassStructures(m).frame(n).stim(o).wRect);
        end
        ...
        %   Cue and target frames with photodiode on
