function [window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODRF_030(Display, vstruct, ClassStructure)
...
for m  = 1:length(ClassStructures)
    for n = 1:length(ClassStructures(m).frame)
        ...
        if n < numel(ClassStructures(m).frame)-1 % not the last frame (target), fixation always ON
