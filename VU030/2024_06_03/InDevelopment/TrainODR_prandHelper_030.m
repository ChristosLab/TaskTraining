...
%%  Version info
version = 'TrainODR_prandHelper_030_v0.11_2024_01_29' ; % after code changes, change version
%   1  : 1 helper/ no helper
...
Lums = [0, 1];
Lums_label = {'no_helper', 'helper'};
for i = 1:n_class
    current_degree = (i - 1)*360/n_class;
    for j = 1:2
    GeneralVars.ClassStructure(2*(i - 1) + j).frame(1).stim.end = stim_radius.*[cos(current_degree/180*pi), sin(current_degree/180*pi)];
    GeneralVars.ClassStructure(2*(i - 1) + j).frame(2).stim.end = stim_radius.*[cos(current_degree/180*pi), sin(current_degree/180*pi)];
    GeneralVars.ClassStructure(2*(i - 1) + j).frame(1).Lum = 1;
    GeneralVars.ClassStructure(2*(i - 1) + j).frame(2).Lum = Lums(j);
    GeneralVars.ClassStructure(2*(i - 1) + j).Notes = [num2str(current_degree), '_', Lums_label{j}];
    end
end

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datain(1:4) = [1 0.5 3.0 0.3];  %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
...
datain(11) = 1.2;               %  Helper luminance as percentage (1 - 100) of color depth (typically 0 - 255)
numBurst = 2;
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODR_030_prandHelper(Display, vstruct, GeneralVars.ClassStructure);
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Save Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
