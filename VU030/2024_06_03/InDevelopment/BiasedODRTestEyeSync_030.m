...
Version = 'BiasedODRTestEyeSync_030' ; % after code changes, change version
%   Added AllData.parameter.script variable to save the current script as
%% Parameters

loc_mean = 37;                 % In degree, change before run
datain(1:4) = [0.5, 0.5, 0.5, 0.2];  % Default waiting times for each frame [fixation, cue, delay, saccade]
...
datain(9) = 50;                 % Radius in degree of target window
...
%         audio_queue = {'', []; '', []; '', []; '', []};
        ...
        AllData.trials(save_counter).EndofTrial_eyetime = trial_eye_timestamp(end);
