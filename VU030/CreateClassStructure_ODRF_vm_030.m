function  [ClassStructure, p, alpha, AllList]= CreateClassStructure_ODRF_vm_030(d, nsample, thetahat, kappa, w) %,Nclass,StartAngle,angleStep)
% clear
if nargin < 4
    w=[0.5 0.5];
end
...
for StartAngle = alpha'
    ...
    ClassStructure(nClass).frame(3).stim.end = [d*cos(angle) d*sin(angle)];
    ClassStructure(nClass).frame(3).stim.feature =6;
