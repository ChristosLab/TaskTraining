...
for m = 1:length(ClassStructure)  %generate the eye location for each frame
    for n = 1:length(ClassStructure(m).frame)
        XY      = ClassStructure(m).frame(n).stim(1).end .* vstruct.pixdeg;   %match location
        XY(2)   = XY(2)*-1;   %nonmatch location
        tCenter = XY + [Display.centerX Display.centerY]; %location are relative to screen center so calculate that
