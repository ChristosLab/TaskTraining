...
% try
    while (BreakState ~= 1) && (blockcounter <= totalblocks)
        ...
        while (repeatcounter <= totaltrials) && (BreakState ~=1)
            ...
            send_network_event(strcat('TTL Line=',num2str(CurrentClass+8),' State=0'));
...
%             clc
            ...
            if Result == 1
                send_network_event(strcat('TTL Line=',num2str(CurrentClass+8),' State=1'));
...
% end
outputSingleScan(DO,[0,0,0,0,0,0,0,0]);
save(['C:\Users\CCLAB\Documents\MATLAB\Behavioral_Data\' savename],'AllData');% clear
%CleanUp
