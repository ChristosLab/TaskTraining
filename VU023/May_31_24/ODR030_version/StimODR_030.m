...
datain(8) = 3.5;                 %  Radius in degree of fixation window
...
%zeroMQwrapper('Send',handle, save_name);
send_network_event(savename);
WaitSecs(2);
% try
    while (BreakState ~= 1) && (blockcounter <= totalblocks)
     ...
     %       send_network_event(strcat('TTL Line=',num2str(ceil(CurrentClass/2)+8),' State=0'));
...
%                 wavesoundplay(audio_queue{frame_idx, 1}, audio_queue{frame_idx, 1});
            ...
            if Result == 1
            %    send_network_event(strcat('TTL Line=',num2str(ceil(CurrentClass/2)+8),' State=1'));
...
%             zeroMQwrapper('Send',tcp_handle , num2str(save_counter));
            %  Intertrial inverval
