      ...
      %  outputcounter = outputcounter + 1;
        ...
        %    Deg = Seq(blockcounter,trialcounter);
            ...
            while (correct_inrepeat <= num_condition) && (BreakState ~=1)
                startBackground(ai);
                pause(0.2);
                eyerecordstart=GetSecs;
                AllData.block(blockcounter).repeat(repeatcounter).trial(finished_trialcounter).eyerecordstart = eyerecordstart; %cue degree
                ...
                if Result == 1
                  %  RectionTime = 0;
                    trial_duration=GetSecs-eyerecordstart;
                    X_EyeData=trial_eye_data(end-round(trial_duration*500)-1:end,1);
                    Y_EyeData=trial_eye_data(end-round(trial_duration*500)-1:end,2);
                    AllData.block(blockcounter).repeat(repeatcounter).trial(finished_trialcounter).EyeData(round(trial_duration*500),1)=trial_eye_data(end-round(trial_duration*500)-1:end,1); %eye sample at 500 Hz
                    AllData.block(blockcounter).repeat(repeatcounter).trial(finished_trialcounter).EyeData(round(trial_duration*500),2)=trial_eye_data(end-round(trial_duration*500)-1:end,2); %eye sample at 500 Hz
           ...
           %         intertrial_interval = 2;
               ...
               %     intertrial_interval = 2;
                ...
                set(hLine(4), 'XData',round(trial_duration*500),'YData', X_EyeData)
                set(hLine(5), 'XData',round(trial_duration*500),'YData', Y_EyeData)
