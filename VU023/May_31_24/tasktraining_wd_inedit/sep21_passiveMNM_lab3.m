...
for n = 1:8
    ...
    sRect(n*45,:) = [dX1+(pixs(n,1)),dY1+(pixs(n,2)),dX2+(pixs(n,1)),dY2+(pixs(n,2))];  %pixel coordinates of conners of 8 stim
    dRect(n*45,1:2) = [newcoors(n,1) newcoors(n,2)];  %degree locations of 8 stim
end
      ...
      %  outputcounter = outputcounter + 1;
        ...
        %    Deg = Seq(blockcounter,trialcounter);
            ...
            while (correct_inrepeat <= num_condition) && (BreakState ~=1)
                outputcounter = outputcounter + 1; 
                temp_pick=unused_index(randperm(length(unused_index),1)); %the index randomly picked in unused_index
                ...
                while 1
                    ...
                    Result = 0;
                    %  Display Fixation
                    Screen('FillRect', window, [1.0 1.0 1.0],[X1,Y1,X2,Y2]);
                    Screen(window,'Flip'); 
                   ...
                   % AllData.block(blockcounter).trial(trialcounter).repeat(repeatcounter).fixOn=GetSecs;%get correct trial fixon time
                    ...
                    while (FixState == 1) && ((GetSecs - breaktime) < frame1)
                      ...
                      if distractor1_state==1 && distractor2_state==0 && distractor1_fliped==0
                        Screen('FillRect', window, [1,0,0; 1,1,1; 1,1,1]',[sRect(loc_list(all_cues(temp_pick)),:);[X1,Y1,X2,Y2];[25,20,45,40]]');
                        Screen(window,'Flip'); 
                        WaitSecs(0.045);
                      ...
                      if distractor1_state==0 && distractor2_state==1 && distractor2_fliped==0
                        Screen('FillRect', window, [1,0,0; 1,1,1; 1,1,1]',[sRect(loc_list(all_samples(temp_pick)),:);[X1,Y1,X2,Y2];[25,20,45,40]]');
                        Screen(window,'Flip'); 
                        WaitSecs(0.045);
                        outputSingleScan(DO,[0,0,0,0,1,0,1,0]);          
