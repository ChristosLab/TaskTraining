...
%%  Version info
version = 'TrainODR_023_v0.11_11_13_21' ; % after code changes, change version
%   TrainODR_023_v0.1 to TrainODR_023_v0.11_11_13_21
%   - edited WaveDisplayParams_023 to correct voltage to eye angle
%     conversion coordinate system conversion
%   - Updated end-of-trial eye trace plotting
...
datain(10) = 15;               %  Stimulus luminance as percentage (1 - 100) of color depth (typically 0 - 255)
    ...
    while (BreakState ~= 1) & (blockcounter <= totalblocks)
        ...
        while (repeatcounter <= totaltrials) & (BreakState ~=1)
            ...
            for frame_idx = 1:numel(window_queue)
                ...
                Screen(window,'Flip');
                timestamp_queue(frame_idx) = GetSecs;
                outputSingleScan(DO,[0,0,0,0,0,0,1,0]);
%                 wavesoundplay(audio_queue{frame_idx, 1}, audio_queue{frame_idx, 1});
                UpdateEyeDisplay(AllCoordinates.fRect, FixWindowSize, AllCoordinates.fRect,vstruct, hLine,'on')
                %
                ...
                else    %   Aquiring new fixation mean +1 Statecode
                    Statecode = Statecode + 1;
                end
                %
