function [window, f, r,WindowStructure, AllCoordinates] = CreateWindowStruct5_old(Display, vstruct, ClassStructure, MNM,numtargets,gray)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Changes %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for m = 1:length(ClassStructure)
    for n = 1:3
        ...
        if n == 3
            for o = 1:2
                XY = ClassStructure(m).frame(n).stim(o).end .* vstruct.pixdeg;
                XY(2) = XY(2)*-1;
                tCenter = XY + [Display.centerX Display.centerY];
    ...
    for n = 1:length(ClassStructures(m).frame)
        ...
        if n < 3
            ...
            for o = 1:numtargets % Display either the first (correct) or both choice targets in the class
                if o == 2
                    Screen(WindowStructure(m).frame(n).end,'FillRect',gray*WhiteIndex(0),ClassStructures(m).frame(n).stim(o).wRect);
