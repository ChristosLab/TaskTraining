function ActiveMNM_phase2(varargin)
%11_3_2021 version
...
daqreset;
clc
load('C:\Users\CCLAB\Documents\MATLAB\inuse\Feature_classesphase1.mat'); %this file for phase 3 and 4

%SizeofStim = 2;% XQ, size 2 degree
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin < 1                      %  If there are no arguments given
    datain(1:4) = [1 .5 0.25 .5];    %  Default waiting times for each frame
    datain(5) = 3;                 %  Trial type
    datain(6) = 10;                %  Number of blocks
    datain(7) = 0;                %  Stimulus eccentricity, not used here
    datain(8) = 25;                 %  Radius in degree of fixation window
    datain(9) = 75;                 %  Radius in degree of target window
    datain(10) = 100;              %  contrast in percentage
    datain(5)=0.25;   %second delay
    datasin = 'fiona';
    numBurst = 1;  
...
matching_choice={'Match','NonMatch'};
current_choice=1;
MNM = matching_choice{current_choice};
Xscale = 3.5;
Xscalecenter = 0;
Yscale = 3.5;
Yscalecenter = 0;
switch_length=3;  %match and nonmatch swith every three finished trials
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             Name Output File 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
version = 'ActivefeatureMNMphase1 3NOV21' ; % after code changes, change version
...
switch_counter=0;%counter to switch match and nonmatch
BreakState = 0;
outputcounter = 0;
save_counter = 1;
correctcounter = 0;
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStruct6(Display, vstruct, GeneralVars.ClassStructure,MNM);
...
%try
    while (BreakState ~= 1) & (blockcounter <= totalblocks)
        ...
        if switch_counter>switch_length
            switch_counter=0;
        ...
        while (repeatcounter <= realtrials) & (BreakState ~=1)            
            ...
            startBackground(ai);        %    flushdata(ai)
    ...
    %        end 
            ...
            while 1
                ...
                Screen(window,'Flip');
                AllData.trials(save_counter).Trialstart = GetSecs;                outputSingleScan(DO,[0,0,0,0,0,0,0,1]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    [FixState_correct] = CheckFixation(degree_fcenter, Display.TargetWindow,Xscale,Yscale,ai);
                    [FixState_wrong] = CheckFixation(degree_fcenter, [1,-1].*Display.TargetWindow,Xscale,Yscale,ai);
                    end                    
                    breaktime = GetSecs;
                    %  Eye must stay within target window for frametime(6)
                    while (FixState_correct|FixState_wrong == 1) & ((GetSecs - breaktime) < frametime(6))
                        DisplayEye(hAxes, hLine, Xscale, Yscale, ai);
                        degree_fcenter=[1,-1].*(AllCoordinates.cCenter(CurrentClass,:,3)-vstruct.res/2).*vstruct.degpix;                         
                        [FixState_correct] = CheckFixation(degree_fcenter, Display.TargetWindow,Xscale,Yscale,ai);
                        [FixState_wrong] = CheckFixation(degree_fcenter, [1,-1].*Display.TargetWindow,Xscale,Yscale,ai);
                   end            
                    if (FixState_correct | FixState_wrong) == 0
                        wavesoundplay('abort.wav',0.8);
                        break;
                    end
                    if FixState_correct==1
                    Statecode = 7;  %correct trials
                    else
                    Statecode = 8;   %error trials 
                    end
