    ...
    for j=1:2  %frame
        if j==2
          for s=1:numtargets %targets
              num_white_pixels = nnz(Simulus(ClassStructure(i).frame(j).stim(s).feature).Feature);
              scale = 10000 / num_white_pixels;
            stimsize(i,j,s,:)=sqrt(scale)*flip(size(Stimulus(ClassStructure(i).frame(j).stim(s).feature).Feature,[1,2]));
          end
        else
              num_white_pixels = nnz(Simulus(ClassStructure(i).frame(j).stim(1).feature).Feature);
              scale = 10000 / num_white_pixels;
            stimsize(i,j,1,:)=sqrt(scale)*flip(size(Stimulus(ClassStructure(i).frame(j).stim(1).feature).Feature,[1,2]));
        end
    end
end
