...
if nargin < 1                      %  If there are no arguments given
   ...
   % datain(5) = 3;                 %  Trial type, not used here
    ...
    switch_threshold = 3;
    burst_amount = 2;
    %Class_touse      = [5:8,13:16,80+[5:8],80+[13:16]];   %first axis
   % Class_touse      = 20+[5:8,13:16,80+[5:8],80+[13:16]];   %second axis
    Class_touse      = 40+[5:8,13:16,80+[5:8],80+[13:16]];   %thrid axis
    %Class_touse      = 60+[5:8,13:16,80+[5:8],80+[13:16]];   %fourth axis
...
GeneralVars.ClassStructure=[GeneralVars.ClassStructure,GeneralVars.ClassStructure];
for i=1:160
    if i<81
        GeneralVars.ClassStructure(i).Stimulation = 0;
    else
        GeneralVars.ClassStructure(i).Stimulation = 1;
    end
end
%%%%%%%%%%%%%%%%%%%%%% Training Variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 388.
        %totaltrials = lcm(length(GeneralVars.ClassStructure),length(Class_touse)); %to create index vecotr tracking what classes were used
        totaltrials=16;
...
%try
    while (BreakState ~= 1) & (blockcounter <= totalblocks)
        ...
        dataout(outputcounter,1:8) = {'Trial' 'Class #' 'Correct' ...
                'Success' 'C-(%)' 'Notes','State','if_stimulation'};
         ...
         %   putvalue(dio,[0 0 0 0 0 0 0 0]); 
            ...
            while 1
                ...
                while (FixState == 1) & ((GetSecs - breaktime) < frametime(3))
                    ...
                    degree_fcenter=[1,-1].*(AllCoordinates.fCenter-vstruct.res/2).*vstruct.degpix;
                    [FixState] = CheckFixation(degree_fcenter, Display.FixationWindow, ai, vstruct);
                    if if_stimulation==1 & stimulation_status==0 & (GetSecs - breaktime)>stimulation_delay
...
%                 APMSendReward(outputcounter-blockcounter,1,APMConn)
                dataout(outputcounter,1:8) = {outputcounter-blockcounter, CurrentClass,correctcounter, 1,p_correct,GeneralVars.ClassStructure(CurrentClass).Notes,Statecode,if_stimulation}
...
%                 APMSendReward(outputcounter-blockcounter,0,APMConn)               
                dataout(outputcounter,1:8) = {outputcounter-blockcounter, CurrentClass, correctcounter, 0,p_correct,GeneralVars.ClassStructure(CurrentClass).Notes,Statecode,if_stimulation}                  
