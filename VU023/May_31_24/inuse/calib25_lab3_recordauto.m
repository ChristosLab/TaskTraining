...
if nargin < 1                      %   If there are no arguments given
    datain(1:4) = [1.5 .1 .1 .1];     %  Default waiting times for each frame
    datain(5) = 3;                 %  Trial type
    datain(6) = 1;                %  Number of blocks
    datain(7) = 10;                %  Stimulus eccentricity
    datain(8) = 50;               %  Radius in degree of fixation window
...
Xscale = 3.5;
Xscalecenter = 0; %positive means left
Yscale = 3.5;
Yscalecenter = 0; %negative means go down
...
%calculate new scale and shift
traindata={AllData.block.trial.repeat};
for i=1:length(traindata)
    temp_data=traindata{i};
    temp_eyeloc={temp_data.eye_loc};
    temp_eyetime={temp_data.eye_time};
    temp_fixin=[temp_data.fixIn];
    trial1_loc=temp_eyeloc{2};
    trial2_loc=temp_eyeloc{3};
    trial1_eyetime=temp_eyetime{2};
    trial2_eyetime=temp_eyetime{3};
    trial1_fixtime=find(trial1_eyetime>temp_fixin(1));
    trial2_fixtime=find(trial2_eyetime>temp_fixin(2));
    meanx(i)=mean([median(trial1_loc(trial1_fixtime,1)),median(trial2_loc(trial2_fixtime,1))]);
    meany(i)=mean([median(trial1_loc(trial1_fixtime,2)),median(trial2_loc(trial2_fixtime,2))]);
   % meanx(i)=median(trial2_loc(trial2_fixtime,1));
   % meany(i)=median(trial2_loc(trial2_fixtime,2));
end
x_shift=(mean(meanx([2,3,4,16,17]))+mean(meanx([8,9,10,11,12])))/2;
y_shift=(mean(meany([4,5,6,7,8]))+mean(meany([12,13,14,15,16])))/2;
x_scale=1/((mean(meanx([2,3,4,16,17]))-mean(meanx([8,9,10,11,12])))/20);
y_scale=1/((mean(meany([4,5,6,7,8]))-mean(meany([12,13,14,15,16])))/20);
new_xscale=x_scale*Xscale;
new_yscale=y_scale*Yscale;
new_xshift=(x_shift+Xscalecenter)/new_xscale;
new_yshift=(y_shift+Yscalecenter)/new_yscale;
disp('new_xscale:');
disp(new_xscale);
disp('new_yscale:');
disp(new_yscale);
