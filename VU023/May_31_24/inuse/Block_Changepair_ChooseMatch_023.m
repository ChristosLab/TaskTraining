function Block_Changepair_ChooseMatch_023(varargin)
%  10/24/22 by WD
...
daqreset;
clc
load('C:\Users\CCLAB\Documents\MATLAB\inuse\CMT_classes_changepair.mat'); %this file for phase 3 and 4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%sc%%%%%%%%%%%%%%%%%%
if nargin < 1                      %  If there are no arguments given
    datain(1:3) = [1 .5 .25];    %  Default waiting times for each frame
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
version = 'CMT023 28Nov2022' ; % after code changes, change version
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStruct_CMT(Display, vstruct, GeneralVars.ClassStructure,numtargets,dist_lum);
...
%try
    while (BreakState ~= 1) & (blockcounter <= totalblocks)
     ...
     %    CurrentClass = SwithchClass(currentindex);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%totaltrial:number of trials in a block
%SwitchClass: Class number for each trial in a block
%IndexTotl: index within SwitchClass that could be used in a sub-block
%IndexHist: track used history of trial usage in one block
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           Randomly choose a trial that could be used
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            cvar = randperm(length(IndexTotl));
            currentindex=IndexTotl(cvar(1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    while (FixState == 1) & ((GetSecs - breaktime) < 0.6)
                        statecode=6;
                        [eyeX, eyeY] = DisplayEye(Display, hAxes, hLine, ai);
                        degree_fcenter=[1,-1].*(AllCoordinates.cCenter(CurrentClass,:,2)-vstruct.res/2).*vstruct.degpix;                         
                        [FixState] = CheckFixation(degree_fcenter, Display.TargetWindow, ai, vstruct);
...
%                 APMSendReward(outputcounter-blockcounter,1,APMConn)
                ...
                IndexHist(currentindex) = currentindex;  %store used trial index
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
