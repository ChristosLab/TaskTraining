function [window, f, r,WindowStructure, AllCoordinates] = cuelum_CreateWindowStruct_CMT_mix(Display, vstruct, ClassStructure,numtargets,cue_lum)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Changes %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lum=[1,1]; %transparency for numtargets
    ...
    for n = 1:length(ClassStructures(m).frame)
        ...
        if n < 2
            for o = 1:length(ClassStructures(m).frame(n))
                ...
                Screen(WindowStructure(m).frame(n).end,'PutImage',cue_lum*Stimulus(ClassStructure(m).frame(n).stim(o).feature).Feature ...
