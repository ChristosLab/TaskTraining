...
stimulation_delay=0.3;
%%  Version info
version = 'TrainODR_023_v0.13_05_09_22' ; % after code changes, change version
%   add stimulation to ODR task
...
for i = 1:n_class
    ...
    GeneralVars.ClassStructure(i).Stimulation = 0;
end
for i = 1:n_class
    current_degree = (i - 1)*360/n_class;
    GeneralVars.ClassStructure(i+n_class).frame(1).stim.end = stim_radius.*[cos(current_degree/180*pi), sin(current_degree/180*pi)];
    GeneralVars.ClassStructure(i+n_class).frame(2).stim.end = stim_radius.*[cos(current_degree/180*pi), sin(current_degree/180*pi)];
    GeneralVars.ClassStructure(i+n_class).Notes = num2str(current_degree);
    GeneralVars.ClassStructure(i+n_class).Stimulation = 1;
end
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datain(1:4) = [1 0.5 1.5 0.2];    %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
...
OutputFileNames = fileoutdlg({'Behavior output file',}, 'Output Files',[1 30],{'TRI_ODR_'});
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODR_023(Display, vstruct, GeneralVars.ClassStructure);
...
% try
    while (BreakState ~= 1) & (blockcounter <= totalblocks)
        ...
        dataout(outputcounter,1:8) = {'Trial' 'Class #' 'Correct' ...
            'Success' '%' 'Notes','State','if_stimulation'};
        IndexHist = zeros(1,totaltrials);
        IndexTotl = randperm(length(GeneralVars.ClassStructure));
        CurrentClass = IndexTotl(1);  %the first trial just random picking
...
%                 wavesoundplay(audio_queue{frame_idx, 1}, audio_queue{frame_idx, 1});
                ...
                if_stimulation=GeneralVars.ClassStructure(CurrentClass).Stimulation;
                breaktime = GetSecs;
                while (FixState == 1) & ((GetSecs - breaktime) < frame_time_queue(frame_idx))
                    DisplayEye(Display, hAxes, hLine);
                    [FixState] = CheckFixation(eye_target_queue{frame_idx, 1}, eye_target_queue{frame_idx, 2}, Display);               
                    if frame_idx>2 & if_stimulation==1 & stimulation_status==0 & (GetSecs - breaktime)>stimulation_delay
                       stimulation_status=1;
                       outputSingleScan(DO,[0,0,0,0,0,0,1,1]);
                       outputSingleScan(DO,[0,0,0,0,0,0,1,0]);
                    end
                end
            ...
            AllData.trials(save_counter).if_stimulation=if_stimulation;
%             clc
            AllData.trials(save_counter).Statecode = Statecode;
            if Result == 1
                AllData.trials(save_counter).Reward = 'Yes';
                correctcounter = correctcounter + 1;
                dataout(outputcounter,1:8) = {outputcounter-blockcounter, CurrentClass,correctcounter, 1, correctcounter/fixation_counter,GeneralVars.ClassStructure(CurrentClass).Notes,Statecode,if_stimulation}
                ...
                dataout(outputcounter,1:8) = {outputcounter-blockcounter, CurrentClass, correctcounter, 0,correctcounter/fixation_counter,GeneralVars.ClassStructure(CurrentClass).Notes,Statecode,if_stimulation}
