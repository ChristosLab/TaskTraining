...
Version = 'TrainODRDistonly_023_v0.1_03_17_22' ; % after code changes, change version

%% Parameters

cue_loc = 2;                 % Change before run. 1 for card, 2 for diag, 3 for both
datain(1:4) = [1, .5, 1.25, 0.3];  % Default waiting times for each frame [fixation, cue, delay, saccade] [1, .5, 1.25, 0.2]
...
datain(11) = 5;              % distractor luminance as percentage (1 - 100) of color depth (typically 0 - 255)
datain(12) = 0;                % Helper luminance as percentage (1 - 100) of color depth (typically 0 - 255)
num_burst = 4;
fix_aquisition = 1;
target_aquisition = 0.5; % 0.6
...
[ClassStructure] = CreateClassStructure_ODR_dist_only_023(stim_radius, cue_loc);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Visual settings
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Stimulus Windows
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructODR_023(Display, vstruct, ClassStructure);
