function TrainFixation_023(varargin)
...
if nargin < 1                      %  If there are no arguments given
    datain(1:4) = [2 1 1.5 .5];    %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
    datain(5) = 3;                 %  Trial type
    datain(6) = 3;                %  Number of blocks
    datain(7) = 0;                %  Stimulus eccentricity
    datain(8) = 3.4;                 %  Radius in degree of fi1xation window
...
OutputFileNames = fileoutdlg({'Behavior output file',}, 'Output Files',[1 30],{'Yuki_'});
