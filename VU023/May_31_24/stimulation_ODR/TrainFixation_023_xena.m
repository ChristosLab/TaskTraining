function TrainFixation_023_xena(varargin)
...
daqreset;
clc
GeneralVars.ClassStructure.frame.stim.end = [0, 0];
GeneralVars.ClassStructure.Notes = 'No target';
GeneralVars.ClassStructure = repmat(GeneralVars.ClassStructure, 1, 100);

% load('C:\Users\CCLAB\Documents\MATLAB\inuse\Feature_classes.mat');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin < 1                      %  If there are no arguments given
    datain(1:4) = [2.5 0 1.5 .5];    %  Default waiting times for each frame [fixation. cue delay1 sample delay2]
...
OutputFileNames = fileoutdlg({'Behavior output file',}, 'Output Files',[1 30],{'Xena_'});
...
version = 'TrainFixation_31August21' ; % after code changes, change version
...
blockcounter = 1;
gate_off_time = 1;
intertrial_interval_correct = 7;
intertrial_interval_error   = 7;
aquisition_time = 4;
black = BlackIndex(0);
ReactionTime = 0;
...
[window, f, r, WindowStructure, AllCoordinates] = CreateWindowStructNull(Display, vstruct, GeneralVars.ClassStructure,MNM);
