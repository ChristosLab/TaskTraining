function [window, f, r,WindowStructure, AllCoordinates] = CreateWindowStructODRreminder_023(Display, vstruct, ClassStructure)
...
for m = 1:length(ClassStructure)
    for n = 1:numel(ClassStructure(m).frame)
        ...
        ClassStructures(m).frame(n).lum = ClassStructure(m).frame(n).lum;
    end
end
...
for m  = 1:length(ClassStructures)
    for n = 1:length(ClassStructures(m).frame)
        ...
        for o = 1:numel(ClassStructures(m).frame(n).stim)
            Screen(WindowStructure(m).frame(n).end,'FillRect',ClassStructures(m).frame(n).lum*WhiteIndex(0),ClassStructures(m).frame(n).stim(o).wRect);
        end
