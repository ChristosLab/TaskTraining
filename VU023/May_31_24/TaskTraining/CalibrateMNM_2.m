function CalibrateMNM_2(varargin)
...
else     % arguments exist from Gui, use them
...
%%%%%%%  This section creates a window to graphically display the analog eye position in realtime.%%%%%%%%%%%%%%
...
try
    while (BreakState ~= 1) && (blockcounter <= totalblocks)
        ...
        while (trialcounter <= totaltrials) && (BreakState ~=1)
            repeatcounter         = 1;
%             outputSingleScan(DIO, [0 0 0 0 0 0 0 1]);
            Deg                   = Seq(blockcounter,trialcounter);
%             flushdata(ai) %% don't required
...
%                     outputSingleScan(DIO, [0 0 0 0 0 0 1 1]);
                    Screen(window,'WaitBlanking'); 
%                     outputSingleScan(DIO, [0 0 0 0 0 0 0 1])3
...
%                     AllData.block(blockcounter).trial(trialcounter).repeat(repeatcounter).fixInEye=timeEye;
                ...
                %%%  If this point in the code is reached, the subject completed the trial successfully and Result is changed from 0 to 1
                %                 outputSingleScan(DIO, [0 0 0 0 0 0 0 0]);
                ...
                if Result == 1
                    ...
                    [AllData.block(blockcounter).trial(trialcounter).repeat(repeatcounter).EyeData(:,1:3), ...
                     AllData.block(blockcounter).trial(trialcounter).repeat(repeatcounter).EyeData(:,4)] = startForeground(AI);
                    ...
                    outputSingleScan(DIO, [1 0 0 0 0 0 0 0]);
...
[AllData.eyedata,AllData.eyetime,AllData.eyeabstime,AllData.eyelog] = startForeground(AI);
close(hFig)
daqreset
