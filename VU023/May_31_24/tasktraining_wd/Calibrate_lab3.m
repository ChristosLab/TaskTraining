...
if nargin < 1                      %   If there are no arguments given
    ...
    datain(8) = 4.5;               %  Radius in pixels of fixation window
    datain(9) = 600;                %  Repeat stimulus this many times
    datain(10) = 127;
    ...
    numBurst = 8;              % Number of bursts received for correct trial
else
...
StimSize = [10 10];                        % size of the stimuli in pixels
...
try
    ...
    WaitSecs(3) %intertrial interval
catch
    lasterr
end
Screen('CloseAll')
...
if any(Breakbutton)
    Breakbuttons = 0;
    disp('Program paused, click once for continue, >2 to exit')
    WaitSecs(1)
    while 1
