function [vstruct, Display] = WaveDisplayParamsODR(vstruct, datain)

pix               = vstruct.siz./vstruct.res;                 % calculates the size of a pixel in cm
%vstruct.degpix    = (2*atan(pix./(2*vstruct.dis))).*(180/pi);    %???why multiply by 2 in old script? size of a pixel in degree
vstruct.degpix    = (atan(pix./vstruct.dis)).*(180/pi); 
vstruct.pixdeg    = 1./vstruct.degpix;                        %how many pixels in one degree
...
%Display.StimSize       = [round(vstruct.pixdeg(1)*(110/140)*1) round(vstruct.pixdeg(2)*(110/140)*1)]; %(100/70) % size of the stimuli in pixels
Display.StimSize       = [round(vstruct.pixdeg(1)) round(vstruct.pixdeg(2))]; % size of the stimuli in pixels
Display.FixSize        = [5 5];
