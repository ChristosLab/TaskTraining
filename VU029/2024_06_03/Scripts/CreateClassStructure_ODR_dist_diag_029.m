...
for j = 1:length(cues)
    ...
    for i = 1:length(dist_angle)
        ClassStructure((j-1)+i).frame(1).stim.end = [stim_eccen*cos(cues(j)/180*pi), stim_eccen*sin(cues(j)/180*pi)];
        ClassStructure((j-1)+i).frame(2).stim.end = [stim_eccen*cos(dist_angle(i)/180*pi), stim_eccen*sin(dist_angle(i)/180*pi)];
        ClassStructure((j-1)+i).frame(3).stim.end = [stim_eccen*cos(cues(j)/180*pi), stim_eccen*sin(cues(j)/180*pi)];
        ClassStructure((j-1)+i).frame(1).stim.feature = 6;
        ClassStructure((j-1)+i).frame(2).stim.feature = 6;
        ClassStructure((j-1)+i).frame(3).stim.feature = 6;
        ClassStructure((j-1)+i).Notes = [num2str(cues(j)) '-' num2str(dist_angle(i))];
    end
end
